...
# db connection
```bash
psql -h localhost -U hos_user -d hos_db

DbPassword123!

-- Check the queue
SELECT id, patient_name, phone_number, status, triggered_at, completed_at, error_message
FROM outbound_call_queue
ORDER BY created_at;

-- Check follow_up_requests (to see what the scheduler picks up)
SELECT id, patient_id, appointment_id, status, scheduled_at
...
```bash
-- Step 1: find valid patient + appointment IDs to use
SELECT a.id AS appointment_id, a.patient_id, a.tenant_id,
...
-- Step 2: insert 3 test follow-up rows (replace the IDs from Step 1)
INSERT INTO follow_up_requests
    (tenant_id, appointment_id, patient_id, reminder_type, minutes_before, status, scheduled_at)
VALUES
    (1, 29, 3, 'CALL', 60, 'PENDING', NOW() + INTERVAL '1 minutes'),
    (1, 30, 3, 'CALL', 60, 'PENDING', NOW() + INTERVAL '1 minutes'),
    (1, 31, 3, 'CALL', 60, 'PENDING', NOW() + INTERVAL '1 minutes');


SELECT id, patient_name, status, triggered_at, completed_at
FROM outbound_call_queue
ORDER BY created_at;    
```